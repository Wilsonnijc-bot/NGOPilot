...
def generate_report(repo: str, release_pr: int, pr_details: list[dict], skipped_deps: list[dict]) -> str:
    """Generate a risk assessment markdown report."""
    lines = [
        f"# Release Risk Assessment — PR #{release_pr}",
        "",
        f"**Repository:** {repo}",
        f"**Total PRs in release:** {len(pr_details) + len(skipped_deps)}",
        f"**Assessed PRs:** {len(pr_details)} (skipped {len(skipped_deps)} dependency-only)",
        "",
    ]
    ...
    # Skipped dependency-only section
    if skipped_deps:
        lines.append("---")
        lines.append("")
        lines.append(f"## Skipped: {len(skipped_deps)} dependency-only PRs")
        lines.append("")
        for pr in skipped_deps:
...
def main():
    ...
    for pr in all_prs:
        if is_deps_only(pr["files"]):
            skipped_deps.append(pr)
            print(f"  [skip] #{pr['number']}: deps-only", file=sys.stderr)
        else:
            pr["risk"] = assess_risk(pr["files"])
            assessed_prs.append(pr)

    skipped_deps.sort(key=lambda x: x["number"])

    print(f"\nAssessed {len(assessed_prs)} PRs, skipped {len(skipped_deps)} dependency-only", file=sys.stderr)

    # Step 5: Generate report
    report = generate_report(args.repo, release_pr, assessed_prs, skipped_deps)
