...
fn create_resource() -> Resource {
    ...
    let mut builder = Resource::builder_empty()
        .with_attributes([
            KeyValue::new("service.name", "ngopilot"),
            KeyValue::new("service.version", env!("CARGO_PKG_VERSION")),
            KeyValue::new("service.namespace", "ngopilot"),
...
fn create_otlp_tracing_layer() -> OtlpResult<OtlpTracingLayer> {
    ...
    let tracer = tracer_provider.tracer("ngopilot");
...
mod tests {
    ...
    fn test_create_resource_defaults() {
        ...
        assert_eq!(get("service.name").as_deref(), Some("ngopilot"));
        assert_eq!(
            get("service.version").as_deref(),
            Some(env!("CARGO_PKG_VERSION"))
        );
        assert_eq!(get("service.namespace").as_deref(), Some("ngopilot"));
    ...
    #[test_case(
        ...
        Resource::builder_empty()
            .with_attributes([KeyValue::new("service.name", "ngopilot"), KeyValue::new("service.version", env!("CARGO_PKG_VERSION")), KeyValue::new("service.namespace", "ngopilot"), KeyValue::new("host.name", session_host()), KeyValue::new("user.name", session_user())])
            .with_detector(Box::new(TelemetryResourceDetector))
            .build();
        "no env vars uses NGOPilot defaults"
    )]
    #[test_case(
        &[("OTEL_SERVICE_NAME", "custom")],
        Resource::builder_empty()
