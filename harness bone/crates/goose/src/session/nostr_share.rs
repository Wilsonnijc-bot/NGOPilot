...
pub fn build_deeplink(nevent: &str, decryption_key: &str) -> String {
    format!(
        "ngopilot://sessions/nostr?nevent={}&key={}",
...
pub fn parse_deeplink(deeplink: &str) -> Result<ParsedShareLink> {
    let parsed = url::Url::parse(deeplink).context("Invalid NGOPilot session share link")?;
    if parsed.scheme() != "ngopilot"
        || parsed.host_str() != Some("sessions")
        || parsed.path() != "/nostr"
    {
        return Err(anyhow!("Invalid NGOPilot Nostr session share link"));
    }
...
mod tests {
    ...
    async fn publish_builds_deeplink_and_encrypted_kind_30278_event() {
        ...
        assert!(share.deeplink.starts_with("ngopilot://sessions/nostr?"));
    ...
    #[test]
    fn parses_deeplink() {
        let parsed = parse_deeplink("ngopilot://sessions/nostr?nevent=abc&key=def").unwrap();
