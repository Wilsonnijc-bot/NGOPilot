...
/// `ngopilot-doc-guide` skill with the resolved docs root. Resolution is
...
fn resolve_docs_root_placeholder(skill: &SourceEntry, content: &str, docs_root: &str) -> String {
    if skill.name != "ngopilot-doc-guide" || skill.source_type != SourceType::BuiltinSkill {
...
mod tests {
    ...
    fn builtin_ngopilot_doc_guide_skill() -> SourceEntry {
        SourceEntry {
            source_type: SourceType::BuiltinSkill,
            name: "ngopilot-doc-guide".to_string(),
            description: "Test docs skill".to_string(),
            content: "Docs root: {{GOOSE_DOCS_ROOT}}.".to_string(),
            path: "builtin://skills/ngopilot-doc-guide".to_string(),
            global: true,
            writable: true,
            supporting_files: Vec::new(),
    ...
    #[test]
    fn resolve_docs_root_placeholder_substitutes_builtin_ngopilot_doc_guide_root() {
        let skill = builtin_ngopilot_doc_guide_skill();
    ...
    #[test]
    fn resolve_docs_root_placeholder_ignores_non_builtin_ngopilot_doc_guide_skills() {
        let mut skill = builtin_ngopilot_doc_guide_skill();
