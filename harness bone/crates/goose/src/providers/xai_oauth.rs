...
async fn poll_device_code_token(device: &DeviceCodeResponse) -> Result<TokenResponse> {
    ...
    loop {
        ...
        match body.error.as_deref() {
            ...
            Some("expired_token") => {
                return Err(anyhow!(
                    "xAI device code expired - please re-run ngopilot configure"
                ));
            }
            other => {
                let detail = body
...
<html>
  <head>
    <title>NGOPilot - xAI Authorization Successful</title>
    <style>
      body {
  ...
  <body>
    <div class="container">
      ...
      <p>You can close this window and return to NGOPilot.</p>
    </div>
...
<html>
  <head>
    <title>NGOPilot - xAI Authorization Failed</title>
    <style>
      body {{
        font-family: system-ui, -apple-system, sans-serif;
...
async fn perform_loopback_oauth_flow(auth_state: &XaiAuthState) -> Result<TokenData> {
    ...
    if webbrowser::open(&auth_url).is_err() {
        tracing::info!(
            "Please open this URL in your browser to authorize NGOPilot with xAI:\n{}",
            auth_url
        );
    }
...
async fn perform_device_code_flow() -> Result<TokenData> {
    ...
    eprintln!(
        "\nTo authorize NGOPilot with xAI, open this URL in any browser:\n  {}\nand enter code: {}\n",
