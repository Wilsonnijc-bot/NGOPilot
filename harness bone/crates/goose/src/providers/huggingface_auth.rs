...
<html>
  <head>
    <title>NGOPilot - Hugging Face Authorization Successful</title>
...
<html>
  <head>
    <title>NGOPilot - Hugging Face Authorization Failed</title>
    <style>
      body {{
        font-family: system-ui, -apple-system, sans-serif;
        display: flex; justify-content: center; align-items: center;
...
async fn perform_loopback_oauth_flow(client_id: &str) -> Result<HuggingFaceTokenData> {
    ...
    if webbrowser::open(&auth_url).is_err() {
        tracing::info!(
            "Please open this URL in your browser to authorize NGOPilot with Hugging Face:\n{}",
            auth_url
        );
    }
