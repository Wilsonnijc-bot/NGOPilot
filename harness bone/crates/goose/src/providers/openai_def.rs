...
pub async fn from_env(
    ...
    //   3. OPENAI_HOST from config file — persisted by `ngopilot configure`
            ...
            match e {
                ConfigError::NotFound(_) => {
                    if config.requires_auth {
                        anyhow::bail!(
                            "Required API key {} is not set. Configure it via `ngopilot configure` or set the {} environment variable.",
                            config.api_key_env,
                            config.api_key_env
                        );
                    }
