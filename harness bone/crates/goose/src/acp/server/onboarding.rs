...
fn import_failure_warning(
    ...
    let source_name = match source_kind {
        OnboardingImportSourceKind::GooseConfig => "NGOPilot configuration",
...
fn scan_goose_config_candidate(path: &Path) -> Option<OnboardingImportCandidate> {
    ...
    if counts.sessions > 0 {
        warnings.push("Sessions are already shared through NGOPilot's data store.".to_string());
    }

    Some(OnboardingImportCandidate {
        id: candidate_id(GOOSE_CONFIG_PREFIX, path),
        source_kind: OnboardingImportSourceKind::GooseConfig,
        display_name: "Existing NGOPilot configuration".to_string(),
...
fn apply_goose_config_candidate(
    ...
    result.warnings.push(
        "Session history already lives in the NGOPilot data store when available.".to_string(),
    );
    Ok(result)
}
...
mod tests {
    ...
    fn apply_onboarding_imports_continues_after_candidate_failure() {
        ...
        assert!(response
            .warnings
            .iter()
            .any(|warning| warning.starts_with("Skipped NGOPilot configuration import at ")));
