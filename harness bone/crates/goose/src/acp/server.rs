...
pub(super) const DEFAULT_PROVIDER_LABEL: &str = "NGOPilot (Default)";
...
impl GooseAcpAgent {
    async fn on_initialize(
        ...
        Ok(InitializeResponse::new(args.protocol_version)
            .agent_info(Implementation::new("ngopilot", env!("CARGO_PKG_VERSION")))
            .agent_capabilities(capabilities)
            .auth_methods(vec![AuthMethod::Agent(
                AuthMethodAgent::new("ngopilot-provider", "Configure Provider")
                    .description("Run `ngopilot configure` to set up your AI provider and API key"),
            )]))
    }
