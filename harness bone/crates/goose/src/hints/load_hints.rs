...
pub const NGOPILOT_HINTS_FILENAME: &str = ".ngopilothints";
...
pub fn get_context_filenames() -> Vec<String> {
    ...
    Config::global()
        ...
        .unwrap_or_else(|_| {
            vec![
                NGOPILOT_HINTS_FILENAME.to_string(),
...
pub fn load_hint_files(
    ...
    if !global_hints_contents.is_empty() {
        hints.push_str("\n### Global Hints\nThese are my global NGOPilot hints.\n");
...
mod tests {
    ...
    fn test_goosehints_when_present() {
        ...
        fs::write(
            dir.path().join(NGOPILOT_HINTS_FILENAME),
            "Test hint content",
        )
        .unwrap();
    ...
    fn test_goosehints_multiple_filenames() {
        ...
        fs::write(
            dir.path().join(NGOPILOT_HINTS_FILENAME),
            "Custom hints file content from .ngopilothints",
        )
        .unwrap();
        ...
        let hints = load_hint_files(
            dir.path(),
            &["CLAUDE.md".to_string(), NGOPILOT_HINTS_FILENAME.to_string()],
            &gitignore,
        );

        assert!(hints.contains("Custom hints file content from CLAUDE.md"));
        assert!(hints.contains("Custom hints file content from .ngopilothints"));
    }
    ...
    fn test_goosehints_configurable_filename() {
        ...
        assert!(!hints.contains(".ngopilothints")); // Make sure it's not loading the default
    }
    ...
    fn test_nested_goosehints_with_git_root() {
        ...
        fs::write(
            project_root.join(NGOPILOT_HINTS_FILENAME),
            "Root hints content",
        )
        .unwrap();
        ...
        fs::write(subdir.join(NGOPILOT_HINTS_FILENAME), "Subdir hints content").unwrap();
        let current_dir = subdir.join("current_dir");
        fs::create_dir(&current_dir).unwrap();
        fs::write(
            current_dir.join(NGOPILOT_HINTS_FILENAME),
    ...
    fn test_nested_goosehints_without_git_root() {
        ...
        fs::write(base_dir.join(NGOPILOT_HINTS_FILENAME), "Base hints content").unwrap();
    ...
    fn test_nested_goosehints_mixed_filenames() {
        ...
        fs::create_dir(&subdir).unwrap();
        fs::write(
            subdir.join(NGOPILOT_HINTS_FILENAME),
            "Subdir .ngopilothints content",
        )
        .unwrap();
        ...
        assert!(hints.contains("Subdir .ngopilothints content"));
    }
...
Additional instructions here."#;
        fs::write(project_root.join(NGOPILOT_HINTS_FILENAME), hints_content).unwrap();
...
End of nested hints"#;
        fs::write(
            components_dir.join(NGOPILOT_HINTS_FILENAME),
...
@../parent.md
End of hints"#;
        fs::write(current_dir.join(NGOPILOT_HINTS_FILENAME), hints_content).unwrap();
...
@../root_file.md
End of hints"#;
        fs::write(subdir.join(NGOPILOT_HINTS_FILENAME), hints_content).unwrap();
        let gitignore = create_dummy_gitignore();

        let hints = load_hint_files(&subdir, &[NGOPILOT_HINTS_FILENAME.to_string()], &gitignore);
    ...
    fn tracker_deduplicates_directories() {
        ...
        let subdir = project_root.join("nested");
        fs::create_dir_all(&subdir).unwrap();
        fs::write(subdir.join(NGOPILOT_HINTS_FILENAME), "nested hints").unwrap();
    ...
    fn tracker_rejects_parent_traversal_outside_working_directory() {
        ...
        fs::write(outside.join(NGOPILOT_HINTS_FILENAME), "outside hints").unwrap();
    ...
    fn tracker_preserves_in_boundary_symlinks_and_deduplicates_aliases() {
        ...
        fs::write(real.join(NGOPILOT_HINTS_FILENAME), "real hints").unwrap();
    ...
    fn tracker_retries_directory_after_parent_is_created() {
        ...
        fs::write(future.join(NGOPILOT_HINTS_FILENAME), "future hints").unwrap();
        tracker.record_tool_arguments(&Some(args), &project_root);
