...
pub fn handle_project_default() -> Result<()> {
    ...
    if projects.is_empty() {
        ...
        if !status.success() {
            println!("Failed to run NGOPilot. Exit code: {:?}", status.code());
        }
        return Ok(());
    }
    ...
    let _ = intro("NGOPilot Project Manager");
    ...
    match choice {
        "resume" => {
            ...
            // Build the command to run NGOPilot
            let mut command = std::process::Command::new(&goose_bin);
            command.arg("session");
        ...
        "fresh" => {
            ...
            // Build the command to run NGOPilot with a fresh session
            let mut command = std::process::Command::new(&goose_bin);
            command.arg("session");
