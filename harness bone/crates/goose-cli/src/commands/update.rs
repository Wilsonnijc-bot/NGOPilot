...
fn asset_name() -> &'static str {
    ...
    {
        "ngopilot-aarch64-apple-darwin.tar.bz2"
    }
    #[cfg(all(target_os = "macos", target_arch = "x86_64"))]
    {
        "ngopilot-x86_64-apple-darwin.tar.bz2"
    }
    #[cfg(all(target_os = "linux", target_arch = "x86_64", target_env = "gnu"))]
    {
        "ngopilot-x86_64-unknown-linux-gnu.tar.bz2"
    }
    #[cfg(all(target_os = "linux", target_arch = "aarch64", target_env = "gnu"))]
    {
        "ngopilot-aarch64-unknown-linux-gnu.tar.bz2"
    }
    #[cfg(all(target_os = "linux", target_arch = "x86_64", target_env = "musl"))]
    {
        "ngopilot-x86_64-unknown-linux-musl.tar.bz2"
    }
    #[cfg(all(target_os = "linux", target_arch = "aarch64", target_env = "musl"))]
    {
        "ngopilot-aarch64-unknown-linux-musl.tar.bz2"
    }
    #[cfg(all(target_os = "windows", target_arch = "x86_64", feature = "cuda"))]
    {
        "ngopilot-x86_64-pc-windows-msvc-cuda.zip"
    }
    #[cfg(all(target_os = "windows", target_arch = "x86_64", not(feature = "cuda")))]
    {
        "ngopilot-x86_64-pc-windows-msvc.zip"
    }
}
...
async fn fetch_attestations_response(
    ...
    let mut req = client
        ...
        .header("User-Agent", "ngopilot-cli");
...
/// Update the NGOPilot binary to the latest release.
///
...
pub async fn update(canary: bool, reconfigure: bool) -> Result<()> {
    ...
    #[cfg(not(feature = "disable-update"))]
    {
        ...
        println!("NGOPilot updated successfully (verified with Sigstore SLSA provenance).");

        // --- Reconfigure if requested -------------------------------------------
        if reconfigure {
            println!("Running ngopilot configure...");
            let status = Command::new(current_exe)
                .arg("configure")
                .status()
                .context("Failed to run ngopilot configure")?;
            if !status.success() {
                eprintln!("Warning: ngopilot configure exited with {status}");
            }
        }
...
///   1. A `ngopilot-package/` subdirectory (Windows releases)
...
fn find_binary(extract_dir: &Path, binary_name: &str) -> Option<PathBuf> {
    // 1. Check ngopilot-package subdir (matches download_cli.sh / download_cli.ps1)
    let package_dir = extract_dir.join("ngopilot-package");
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        if old_exe.exists() {
            fs::remove_file(&old_exe).with_context(|| {
                format!(
                    "Failed to remove old backup {}. Is another NGOPilot process running?",
                    old_exe.display()
                )
            })?;
        }
        ...
        fs::rename(current_exe, &old_exe).with_context(|| {
            format!(
                "Failed to rename running binary to {}. Try closing NGOPilot Desktop if it's open.",
                old_exe.display()
            )
        })?;
...
mod tests {
    ...
    fn test_asset_name_valid() {
        ...
        assert!(name.starts_with("ngopilot-"));
        #[cfg(target_os = "windows")]
    ...
    fn test_binary_name() {
        ...
        #[cfg(target_os = "windows")]
        assert_eq!(name, "ngopilot.exe");
        #[cfg(not(target_os = "windows"))]
        assert_eq!(name, "ngopilot");
    }

    #[test]
    fn test_find_binary_in_package_subdir() {
        let tmp = tempdir().unwrap();
        let pkg = tmp.path().join("ngopilot-package");
    ...
    fn test_replace_binary_windows_rename_away() {
        let tmp = tempdir().unwrap();
        let current = tmp.path().join("ngopilot.exe");
    ...
    fn test_extract_zip_with_package_dir() {
        ...
        // Create a zip in memory with ngopilot-package/ structure
        let mut buf = Vec::new();
        {
            ...
            writer.add_directory("ngopilot-package/", options).unwrap();
            writer
                .start_file("ngopilot-package/ngopilot.exe", options)
                .unwrap();
            writer.write_all(b"fake NGOPilot binary").unwrap();
            writer
                .start_file("ngopilot-package/libtest.dll", options)
                .unwrap();
        ...
        let binary = find_binary(tmp.path(), "ngopilot.exe");
        assert!(binary.is_some());

        let content = fs::read_to_string(binary.unwrap()).unwrap();
        assert_eq!(content, "fake NGOPilot binary");

        // DLL should be in ngopilot-package too
        assert!(tmp.path().join("ngopilot-package/libtest.dll").exists());
    }
    ...
    fn test_validate_entry_path_accepts_safe_paths() {
        assert!(validate_entry_path(Path::new("ngopilot")).is_ok());
        assert!(validate_entry_path(Path::new("ngopilot-package/ngopilot")).is_ok());
    ...
    fn test_extract_tar_bz2_safe_archive() {
        ...
        let mut builder_buf = Vec::new();
        {
            ...
            let data = b"NGOPilot binary content";
            let mut header = tar::Header::new_gnu();
            header.set_size(data.len() as u64);
            header.set_mode(0o755);
            header.set_cksum();
            builder
                .append_data(&mut header, "ngopilot-package/ngopilot", &data[..])
                .unwrap();
        ...
        let extracted = tmp.path().join("ngopilot-package/ngopilot");
        assert!(extracted.exists());
        assert_eq!(
            fs::read_to_string(extracted).unwrap(),
            "NGOPilot binary content"
        );
    }
