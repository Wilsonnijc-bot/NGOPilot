...
pub async fn handle_info(verbose: bool, check: bool) -> Result<()> {
    ...
    println!("{}", style("NGOPilot Version:").cyan().bold());
    ...
    if verbose {
        println!("\n{}", style("NGOPilot Configuration:").cyan().bold());
        let values = config.all_values()?;
        if values.is_empty() {
            println!("  No configuration values set");
            println!(
                "  Run '{}' to configure NGOPilot",
    ...
    if check {
        ...
        match &result {
            ...
            Err(ProviderCheckError::NotConfigured { label, error }) => {
                ...
                print_aligned(
                    "Hint:",
                    &format!("Run '{}'", style("ngopilot configure").cyan()),
                    label_padding,
                );
            }
        ...
        // checks, health probes) can rely on `ngopilot info --check` as a
        // pre-flight verifier.
        if result.is_err() {
