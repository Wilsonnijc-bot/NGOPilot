...
const TUI_NPM_SPEC_ENV: &str = "NGOPILOT_TUI_NPM_SPEC";
const TUI_REL_PATH: &str = "ui/text/dist/tui.js";
const DEFAULT_NPM_SPEC: &str = "@ngopilot/cli@latest";
const NPM_BIN_NAME: &str = "ngopilot-tui";
...
pub fn handle_tui(args: Vec<String>) -> Result<()> {
    ...
    let goose_binary =
        std::env::current_exe().context("could not determine current NGOPilot executable")?;

    let mut cmd = build_command(&source, &args)?;
    cmd.env("NGOPILOT_BINARY", &goose_binary);
...
mod tests {
    ...
    fn find_local_script_ignores_unrelated_directories() {
        ...
        let executable = temp_dir.path().join("install/bin/ngopilot");
    ...
    fn find_local_script_accepts_executable_ancestor() {
        let temp_dir = tempfile::tempdir().expect("create temp dir");
        let executable = temp_dir.path().join("target/debug/ngopilot");
