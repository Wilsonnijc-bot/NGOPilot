//! Deterministic, Rust-driven orchestration for `ngopilot review`.
//!
...
//!
//! - One subprocess per check (`ngopilot run -q -t <prompt>`)
...
/// Run all discovered checks concurrently as `ngopilot run` subprocesses.
///
...
pub async fn run_checks_in_parallel(
    ...
    while let Some(joined) = set.join_next().await {
        let (idx, check, result, quiet) = match joined {
            Ok(v) => v,
            Err(e) => {
                eprintln!("ngopilot review: check task panicked: {e}");
                continue;
            }
        };
        ...
        match result {
            Ok(findings) => {
                if !quiet {
                    eprintln!(
                        "ngopilot review: check '{}' completed: {} finding(s)",
                        check.name,
                        findings.len()
                    );
                }
            ...
            Err(e) => {
                ...
                eprintln!("ngopilot review: check '{}' failed: {e}", check.name);
                results[idx] = Vec::new();
            }
        }
    }
...
///
/// Uses `ngopilot review --turn-limit` when set, otherwise
...
/// Spawn a single `ngopilot run` subprocess for one check and parse its
...
/// Generic `ngopilot run` subprocess that hands a prompt to the model
...
async fn run_subprocess_for_findings(
    ...
    let goose_bin = std::env::current_exe().context("locate current NGOPilot binary")?;
    ...
    if let Some(mut stdin) = child.stdin.take() {
        ...
        // Closing stdin signals EOF to `ngopilot run -i -`.
        drop(stdin);
    }
...
pub async fn run_main_pass_in_parallel(
    ...
    while let Some(joined) = set.join_next().await {
        let (idx, path, result, quiet) = match joined {
            Ok(v) => v,
            Err(e) => {
                eprintln!("ngopilot review: main-pass task panicked: {e}");
                continue;
            }
        };
        match result {
            Ok(raw) => {
                ...
                if !quiet {
                    eprintln!(
                        "ngopilot review: main pass on '{}' completed: {} finding(s)",
                        path,
                        findings.len()
                    );
                }
            ...
            Err(e) => {
                ...
                eprintln!("ngopilot review: main pass on '{}' failed: {e}", path);
...
/// cap enforced by NGOPilot. Without this, models routinely burn turns on
...
fn build_subprocess_turn_budget_section(max_turns: usize) -> String {
    format!(
        "## Turn budget\n\n\
         ...
         NGOPilot enforces this via `--max-turns`; when you exhaust it, the session stops and \
