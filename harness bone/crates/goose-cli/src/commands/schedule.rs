...
pub async fn handle_schedule_services_status() -> Result<()> {
    ...
    println!(
        "The built-in scheduler runs within the NGOPilot process and requires no external services."
    );
    Ok(())
}
...
pub async fn handle_schedule_cron_help() -> Result<()> {
    println!("📅 Cron Expression Guide for NGOPilot Scheduler");
    println!("===========================================\\n");
    ...
    println!("💡 EXAMPLES:");
    println!(
        "  ngopilot schedule add --schedule-id hourly-report --cron \"0 * * * *\" --recipe-source report.yaml"
    );
    println!(
        "  ngopilot schedule add --schedule-id daily-backup --cron \"@daily\" --recipe-source backup.yaml"
    );
    println!("  ngopilot schedule add --schedule-id weekly-summary --cron \"0 9 * * 1\" --recipe-source summary.yaml");
