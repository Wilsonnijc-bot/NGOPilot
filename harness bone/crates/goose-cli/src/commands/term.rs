...
alias @ngopilot='{goose_bin} term run'

ngopilot_preexec() {
    [[ "$1" =~ ^ngopilot\ term ]] && return
    [[ "$1" =~ ^@ngopilot($|[[:space:]]) ]] && return
    ('{goose_bin}' term log "$1" &) 2>/dev/null
}

if [[ -z "$ngopilot_preexec_installed" ]]; then
    ngopilot_preexec_installed=1
    trap 'ngopilot_preexec "$BASH_COMMAND"' DEBUG
fi{command_not_found_handler}"#,
    command_not_found: Some(
        r#"

command_not_found_handle() {
    echo "Command '$1' not found. Asking NGOPilot..."
    '{goose_bin}' term run "$@"
    return 0
}"#,
    ),
...
add-zsh-hook preexec ngopilot_preexec{command_not_found_handler}"#,
    command_not_found: Some(
        r#"
...
function @ngopilot; {goose_bin} term run $argv; end

function ngopilot_preexec --on-event fish_preexec
    string match -q -r '^ngopilot term' -- $argv[1]; and return
    string match -q -r '^@ngopilot($|\s)' -- $argv[1]; and return
...
def --wrapped @ngopilot [...args] { run-external "{goose_bin}" "term" "run" ...$args }

if (($env | get -o NGOPILOT_NU_PREEXEC_INSTALLED | default false) != true) {
    $env.NGOPILOT_NU_PREEXEC_INSTALLED = true
...
$env.config.hooks.command_not_found = {|command_name|
    ...
    print $"Command '($command_name)' not found. Asking NGOPilot..."
...
static POWERSHELL_CONFIG: ShellConfig = ShellConfig {
    script_template: r#"$env:AGENT_SESSION_ID = "{session_id}"
function @ngopilot {{ & '{goose_bin}' term run @args }}

Set-PSReadLineKeyHandler -Chord Enter -ScriptBlock {{
    $line = $null
    [Microsoft.PowerShell.PSConsoleReadLine]::GetBufferState([ref]$line, [ref]$null)
    if ($line -notmatch '^ngopilot term' -and $line -notmatch '^@ngopilot($|\s)') {{
...
pub async fn handle_term_init(
    ...
    let session = match named_session {
        ...
        None => {
            let session = session_manager
                .create_session(
                    working_dir,
                    "NGOPilot Term Session".to_string(),
    ...
    let goose_bin = std::env::current_exe()
        ...
        .unwrap_or_else(|_| "ngopilot".to_string());
...
pub async fn handle_term_log(command: String) -> Result<()> {
    let session_id = std::env::var("AGENT_SESSION_ID").map_err(|_| {
        anyhow!(
            "AGENT_SESSION_ID not set. Initialize terminal integration with `ngopilot term init <shell>` and reload your shell first."
        )
    })?;
    ...
    let session_id = std::env::var("AGENT_SESSION_ID").map_err(|_| {
        anyhow!(
            "AGENT_SESSION_ID not set.\n\n\
             Initialize terminal integration with `ngopilot term init <shell>` in your shell profile, \
...
/// Handle `ngopilot term info` - print compact session info for prompt integration
pub async fn handle_term_info() -> Result<()> {
    ...
    let model_name = config
        ...
        .map(|name| {
            ...
            if let Some(stripped) = short.strip_prefix("ngopilot-") {
                stripped.to_string()
            } else {
                short.to_string()
            }
...
mod tests {
    ...
    fn render_term_init_script_includes_nushell_hooks() {
        let script = render_term_init_script(Shell::Nu, "session-123", "/tmp/ngopilot", false);

        assert!(script.contains("$env.AGENT_SESSION_ID = \"session-123\""));
        assert!(script.contains("def --wrapped @ngopilot [...args]"));
        assert!(!script.contains("def --wrapped @g [...args]"));
        assert!(script.contains("NGOPILOT_NU_PREEXEC_INSTALLED"));
        assert!(script.contains("$env.config.hooks.pre_execution"));
        assert!(script.contains("job spawn { run-external \"/tmp/ngopilot\" \"term\" \"log\" $line | complete | ignore } | ignore"));
        assert!(!script.contains("command_not_found = {|command_name|"));
    }

    #[test]
    fn render_term_init_script_includes_nushell_default_handler() {
        let script = render_term_init_script(Shell::Nu, "session-123", "/tmp/ngopilot", true);

        assert!(script.contains("$env.config.hooks.command_not_found = {|command_name|"));
        assert!(script.contains(
            "run-external \"/tmp/ngopilot\" \"term\" \"run\" $prompt | complete | ignore"
        ));
    }

    #[test]
    fn render_term_init_script_skips_unsupported_default_handler() {
        let script = render_term_init_script(Shell::Fish, "session-123", "/tmp/ngopilot", true);
