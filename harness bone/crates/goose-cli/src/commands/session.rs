...
pub async fn handle_session_export(
    ...
    if nostr {
        return Err(anyhow::anyhow!("NGOPilot was not built with nostr support"));
    }
...
pub async fn handle_session_import(input: String, nostr: bool) -> Result<()> {
    let json = if nostr || input.starts_with("ngopilot://sessions/nostr") {
        #[cfg(feature = "nostr")]
        {
    ...
    let label = match format {
        goose::session::import_formats::ImportFormat::Goose => "NGOPilot",
