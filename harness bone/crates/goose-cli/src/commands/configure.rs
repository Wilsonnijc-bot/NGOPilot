...
pub async fn handle_configure() -> anyhow::Result<()> {
    if !std::io::stdin().is_terminal() {
        anyhow::bail!(
            "ngopilot configure requires an interactive terminal.\n\
             If you installed via 'curl ... | bash', run 'ngopilot configure' separately after installation."
        );
    }
...
pub fn configure_telemetry_consent_dialog() -> anyhow::Result<bool> {
    ...
    println!();
    println!("{}", style("Help improve NGOPilot").bold());
    println!();
    println!(
        "{}",
        style("Would you like to help improve NGOPilot by sharing anonymous usage data?").dim()
    );
    println!(
        "{}",
        style("This helps us understand how NGOPilot is used and identify areas for improvement.")
            .dim()
    );
    println!();
    ...
    println!("{}", style("  • NGOPilot version and install method").dim());
    ...
    println!(
        "{}",
        style("or any personal data. You can change this anytime with 'ngopilot configure'.").dim()
    );
    println!();

    let enabled = cliclack::confirm("Share anonymous usage data to help improve NGOPilot?")
        .initial_value(true)
        .interact()?;
    ...
    if enabled {
        let _ = cliclack::log::success("Thank you for helping improve NGOPilot!");
    } else {
...
async fn handle_first_time_setup(config: &Config) -> anyhow::Result<()> {
    ...
    println!(
        "{}",
        style("Welcome to NGOPilot! Let's get you set up.").dim()
    );
    println!(
        "{}",
    ...
    println!();
    cliclack::intro(style(" ngopilot-configure ").on_cyan().black())?;
...
async fn handle_existing_config() -> anyhow::Result<()> {
    ...
    let action = cliclack::select("What would you like to configure?")
        ...
        .item(
            "settings",
            "NGOPilot settings",
            "Set the NGOPilot mode, Tool Output, Tool Permissions, Experiment, ngopilot recipe github repo and more",
        )
        .interact()?;
...
/// Configure extensions that can be used with NGOPilot
...
pub fn configure_extensions_dialog() -> anyhow::Result<()> {
    let extension_type = cliclack::select("What type of extension would you like to add?")
        .item(
            "built-in",
            "Built-in Extension",
            "Use an extension that comes with NGOPilot",
        )
        .item(
            "stdio",
...
pub async fn configure_settings_dialog() -> anyhow::Result<()> {
    ...
    let setting_type = setting_select
        ...
        .item(
            "recipe",
            "ngopilot recipe github repo",
            "NGOPilot will pull recipes from this repo if not found locally.",
        )
        .interact()?;
...
pub fn configure_goose_mode_dialog() -> anyhow::Result<()> {
    ...
    let mode = cliclack::select("Which NGOPilot mode would you like to configure?")
        .item(
            GooseMode::Auto,
            "Auto Mode",
...
pub fn configure_telemetry_dialog() -> anyhow::Result<()> {
    ...
    if enabled {
        cliclack::outro("Telemetry enabled - thank you for helping improve NGOPilot!")?;
    } else {
...
pub fn configure_keyring_dialog() -> anyhow::Result<()> {
    ...
    match storage_option {
        "keyring" => {
            ...
            let _ = cliclack::log::info(
                "You may need to restart NGOPilot for this change to take effect",
            );
        }
        "file" => {
...
/// Configure experiment features that can be used with NGOPilot
...
fn configure_recipe_dialog() -> anyhow::Result<()> {
    ...
    let mut recipe_repo_input = cliclack::input(
        "Enter your ngopilot recipe GitHub repo (owner/repo): eg: my_org/ngopilot-recipes",
    )
    .required(false);
...
pub fn configure_max_turns_dialog() -> anyhow::Result<()> {
    ...
    cliclack::outro(format!(
        "Set maximum turns to {} - NGOPilot will ask for input after {} consecutive actions",
        max_turns, max_turns
    ))?;
    ...
    match create("openrouter", Vec::new()).await {
        Ok(provider) => {
            let test_result = provider
                .complete(
                    &model_config,
                    "You are NGOPilot, an AI assistant.",
            ...
            match test_result {
                Ok(_) => {
                    ...
                    cliclack::outro("OpenRouter setup complete! You can now use NGOPilot.")?;
                }
                Err(e) => {
        ...
        Ok(provider) => {
            ...
            match test_result {
                Ok(_) => {
                    ...
                    cliclack::outro(
                        "Tetrate Agent Router Service setup complete! You can now use NGOPilot.",
                    )?;
                }
                Err(e) => {
                    eprintln!("⚠️  Configuration test failed: {}", e);
