    ...
    match generate_deeplink(recipe_name, params_map) {
        Ok((deeplink_url, recipe)) => match opener(&deeplink_url) {
            Ok(_) => {
                writeln!(
                    out,
                    "{} Opened recipe '{}' in NGOPilot Desktop",
                    style("✓").green().bold(),
                    recipe.title
                )?;
                Ok(())
            ...
            Err(err) => {
                writeln!(
                    out,
                    "{} Failed to open recipe in NGOPilot Desktop: {}",
                    style("✗").red().bold(),
                    err
                )?;
                writeln!(out, "Generated deeplink: {}", deeplink_url)?;
                writeln!(out, "You can manually copy and open the URL above, or ensure NGOPilot Desktop is installed.")?;
...
fn generate_deeplink(
    ...
    match recipe_deeplink::encode(&recipe) {
        Ok(encoded) => {
            let mut full_url = format!("ngopilot://recipe?config={}", encoded);
...
"#;
    ...
    fn test_handle_deeplink_valid_recipe() {
        ...
        assert!(url.starts_with("ngopilot://recipe?config="));
        let encoded_part = url.strip_prefix("ngopilot://recipe?config=").unwrap();
    ...
    fn test_handle_open_opener_fails() {
        ...
        assert!(result.is_err());
        assert!(output.contains("Failed to open recipe in NGOPilot Desktop"));
