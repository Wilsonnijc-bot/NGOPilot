...
/// Configuration for building a new ngopilot session
///
...
async fn load_extensions(
    ...
    for (id, err) in failed {
        ...
        eprintln!(
            "{}",
            style(format!(
                "  Hint: once the session starts, ask NGOPilot to help debug the '{}' extension",
                label
            ))
            .dim()
        );
...
fn resolve_provider_and_model(
    ...
    let provider_name = session_config
        ...
        .unwrap_or_else(|| {
            output::render_error("No provider configured. Run 'ngopilot configure' first.");
            process::exit(1);
        });
    ...
    let model_name = session_config
        ...
        .unwrap_or_else(|| {
            output::render_error("No model configured. Run 'ngopilot configure' first.");
            process::exit(1);
        });
                    ...
                    && is_provider_unavailable_error(&e) =>
            {
                ...
                match create(&fallback_provider, extensions_for_provider.clone()).await {
                    ...
                    Err(e2) => {
                        output::render_error(&format!(
                        "Error {}.\n\
                        Please check your system keychain and run 'ngopilot configure' again.\n\
