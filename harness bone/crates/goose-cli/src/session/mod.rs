...
impl CliSession {
                    ...
                    None => {
                        output::render_error(
                            "No editor found. Set one with:\n  \
                                 ngopilot configure set goose_prompt_editor \"vim\"\n  \
    ...
    async fn handle_goose_mode(&self, mode: &str) -> Result<()> {
        ...
        output::goose_mode_message(&format!("NGOPilot mode set to '{mode}'"));
        Ok(())
    }
            ...
            Err(e) => {
                output::render_error(&format!(
                    "Cannot switch to provider '{}': {}\n\
                         Set credentials via `ngopilot configure` or the appropriate environment variable.\n\
        ...
        match planner_response_type {
            PlannerResponseType::Plan => {
                ...
                if should_act {
                    ...
                    self.run_mode = RunMode::Normal;
                    // set NGOPilot mode: auto if that isn't already the case
                    let config = Config::global();
...
fn prompt_tool_confirmation(security_prompt: &Option<String>) -> Result<Permission> {
    ...
    let prompt = if let Some(security_message) = security_prompt {
        ...
        "NGOPilot would like to call the above tool, do you allow?".to_string()
    };
...
async fn get_reasoner(
    ...
    let provider = if let Ok(provider) = config.get_param::<String>("GOOSE_PLANNER_PROVIDER") {
        ...
        config
            .get_goose_provider()
            .expect("No provider configured. Run 'ngopilot configure' first")
    };
    ...
    let model = if let Ok(model) = config.get_param::<String>("GOOSE_PLANNER_MODEL") {
        ...
        config
            .get_goose_model()
            .expect("No model configured. Run 'ngopilot configure' first")
    };
