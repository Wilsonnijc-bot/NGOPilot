...
fn main() -> Result<()> {
    ...
    let handle = std::thread::Builder::new()
        .name("ngopilot-cli-main".to_string())
        ...
        .map_err(|e| anyhow::anyhow!("Failed to spawn the NGOPilot CLI main thread: {}", e))?;

    handle
        .join()
        .map_err(|_| anyhow::anyhow!("NGOPilot CLI main thread panicked"))?
}
