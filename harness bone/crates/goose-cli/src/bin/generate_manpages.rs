//! Generate manpages for the NGOPilot CLI.
//!
...
//! offline documentation accessible via the `man` command (e.g., `man ngopilot`).
//!
//! When NGOPilot is packaged for Linux distributions (deb, rpm, etc.), the generated
...
//!
//! Output: target/man/ngopilot.1, target/man/ngopilot-session.1, etc.
...
fn generate_see_also(
    ...
    // Always reference the main NGOPilot command if we're not it
    if current_name != "ngopilot" {
        references.push("ngopilot".to_string());
    }

    // Reference parent command if exists and not already added
    if let Some(parent) = parent_name {
        if parent != "ngopilot" && !references.contains(&parent.to_string()) {
    ...
    if current_name == "ngopilot" {
        ...
        for subcmd in cmd.get_subcommands() {
            ...
            if subcmd_name != "help" && !subcmd.is_hide_set() {
                let full_name = format!("ngopilot-{}", subcmd_name);
