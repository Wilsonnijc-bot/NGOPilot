...
const NGOPILOT_SERVER_SECRET_KEY_ENV: &str = "NGOPILOT_SERVER__SECRET_KEY";
...
#[derive(Parser)]
#[command(name = "ngopilot", author, version, display_name = "", about, long_about = None)]
pub struct Cli {
    #[command(subcommand)]
...
pub struct SessionOptions {
    ...
    #[arg(
        ...
        long_help = "Run extensions (stdio and built-in) inside the specified container. The extension must exist in the container. For built-in extensions, NGOPilot must be installed inside the container."
    )]
...
pub struct ExtensionOptions {
    ...
    #[arg(
        ...
        long_help = "Add one or more builtin extensions that are bundled with NGOPilot by specifying their names, comma-separated",
        value_delimiter = ','
    )]
...
pub struct InputOptions {
    ...
    #[arg(
        ...
        long = "text",
        value_name = "TEXT",
        help = "Input text to provide to NGOPilot directly",
        long_help = "Input text containing commands for NGOPilot. Use this in lieu of the instructions argument.",
    ...
    #[arg(
        ...
        help = "Dynamic parameters (e.g., --params username=alice --params channel_name=ngopilot-channel)",
...
enum SessionCommand {
    ...
    Export {
        ...
        #[arg(
            long = "nostr",
            help = "Publish the JSON session export as an encrypted Nostr event and print an NGOPilot share link"
        )]
        nostr: bool,
    ...
    Import {
        #[arg(
            help = "Path to an NGOPilot session export, a Claude Code, Codex, or Pi .jsonl transcript, or an ngopilot://sessions/nostr share link"
        )]
        input: String,
...
#[derive(Subcommand)]
enum SkillsCommand {
    /// List all skills available to the NGOPilot agent
    #[command(about = "List all skills available to the NGOPilot agent")]
    List,
}
...
enum RecipeCommand {
    ...
    /// Open a recipe in NGOPilot Desktop
    #[command(about = "Open a recipe in NGOPilot Desktop")]
    Open {
...
#[derive(Subcommand)]
enum Command {
    /// Configure NGOPilot settings
    #[command(about = "Configure NGOPilot settings")]
    Configure {},

    /// Display NGOPilot configuration information
    #[command(about = "Display NGOPilot information")]
    Info {
    ...
    #[command(about = "Check that your NGOPilot setup is working")]
    Doctor {},

    /// Manage system prompts and behaviors
    #[command(about = "Run one of the mcp servers bundled with NGOPilot")]
    Mcp {
        #[arg(value_parser = clap::value_parser!(McpCommand))]
        server: McpCommand,
    },

    /// Run NGOPilot as an ACP (Agent Client Protocol) agent
    #[command(about = "Run NGOPilot as an ACP agent server on stdio")]
    Acp {
    ...
    Serve {
        ...
        #[arg(
            ...
            help = "Start the ACP endpoint without requiring NGOPILOT_SERVER__SECRET_KEY"
        )]
    ...
    /// Update the NGOPilot CLI version
    #[cfg(feature = "update")]
    #[command(about = "Update the NGOPilot CLI version")]
    Update {
        ...
        #[arg(
            ...
            long_help = "Update to the latest canary version of the NGOPilot CLI, otherwise updates to the latest stable version."
        )]
        canary: bool,

        /// Enforce to re-configure NGOPilot during update
        #[arg(short, long, help = "Enforce to re-configure NGOPilot during update")]
        reconfigure: bool,
    },

    /// Terminal-integrated session (one session per terminal)
    #[command(
        about = "Terminal-integrated NGOPilot session",
        long_about = "Runs an NGOPilot session tied to your terminal window.\n\
                      Each terminal maintains its own persistent session that resumes automatically.\n\n\
                      Setup:\n  \
                        eval \"$(ngopilot term init zsh)\"  # zsh/bash\n  \
                        let init = ($nu.cache-dir | path join \"ngopilot-term-init.nu\"); ^ngopilot term init nu | save --force $init; source $init\n\n\
                      Usage:\n  \
                        ngopilot term run \"list files in this directory\"\n  \
                        @ngopilot \"create a python script\"  # terminal command"
    )]
    Term {
        #[command(subcommand)]
        command: TermCommand,
    ...
    /// Launch the NGOPilot terminal UI (TUI)
    #[cfg(feature = "tui")]
    #[command(
        about = "Launch the NGOPilot terminal UI",
        long_about = "Launch the NGOPilot terminal UI.\n\
                      \n\
                      Resolution order:\n  \
                      1. A local checkout's ui/text/dist/tui.js (dev workflow)\n  \
                      2. `npx --yes --package <spec> -- ngopilot-tui` (deployed installs)\n\
                      \n\
                      Override the npm spec via NGOPILOT_TUI_NPM_SPEC (default: @ngopilot/cli@latest).\n\
    ...
    /// runs the review through NGOPilot.
    #[command(about = "Review the current diff using NGOPilot")]
    Review {
        ...
        /// orchestrator dispatches one `ngopilot run` subprocess per check
...
enum TermCommand {
    ...
    #[command(
        ...
        long_about = "Prints shell configuration to set up terminal-integrated sessions.\n\
                      Each terminal gets a persistent NGOPilot session that automatically resumes.\n\n\
                      Setup:\n  \
                        echo 'eval \"$(ngopilot term init zsh)\"' >> ~/.zshrc\n  \
                        source ~/.zshrc\n\n\
                        Nushell:\n  \
                        let init = ($nu.cache-dir | path join \"ngopilot-term-init.nu\")\n  \
                        ^ngopilot term init nu | save --force $init\n  \
                        source $init\n\n\
                      With --default (anything typed that isn't a command goes to NGOPilot):\n  \
                        echo 'eval \"$(ngopilot term init zsh --default)\"' >> ~/.zshrc\n  \
                        ^ngopilot term init nu --default | save --force $init"
    )]
    Init {
        ...
        /// Make NGOPilot the default handler for unknown commands
        #[arg(
            long = "default",
            help = "Make NGOPilot the default handler for unknown commands",
            long_help = "When enabled, anything you type that isn't a valid command will be sent to NGOPilot. Supported for zsh, bash, and nu."
        )]
        default: bool,
    },
    ...
    #[command(
        ...
        long_about = "Run a prompt in the terminal-integrated session.\n\n\
                      Examples:\n  \
                        ngopilot term run list files in this directory\n  \
                        @ngopilot list files  # terminal command"
    )]
    Run {
        /// The prompt to send to NGOPilot (multiple words allowed without quotes)
...
async fn handle_serve_command(args: ServeCommandArgs) -> Result<()> {
    ...
    let env_secret = std::env::var(NGOPILOT_SERVER_SECRET_KEY_ENV)
        .ok()
    ...
    if !require_token && !dangerously_unauthenticated {
        anyhow::bail!(
            "{NGOPILOT_SERVER_SECRET_KEY_ENV} must be set to start `ngopilot serve`; pass --dangerously-unauthenticated to run without ACP authentication"
        );
    }
    if dangerously_unauthenticated && !require_token {
        warn!(
            "{NGOPILOT_SERVER_SECRET_KEY_ENV} is not set and --dangerously-unauthenticated was passed; the ACP endpoint will accept unauthenticated connections"
        );
    }
    ...
    match (
        ...
        (Some(file), _, _) => {
            let contents = std::fs::read_to_string(file).unwrap_or_else(|err| {
                eprintln!(
                    "Instruction file not found — did you mean to use ngopilot run --text?\n{}",
                    err
                );
                std::process::exit(1);
            });
            ...
            for model in &results {
                ...
                for variant in &model.variants {
                    ...
                    if variant.supported {
                        println!(
                            "    Download: ngopilot local-models download '{}'",
                            variant.download_id
                        );
                    }
                }
...
mod tests {
    ...
    fn completion_command_accepts_nushell_alias() {
        let cli = Cli::try_parse_from(["ngopilot", "completion", "nushell"]).expect("parse failed");
    ...
    fn nushell_completion_generation_emits_module() {
        ...
        CompletionShell::Nu.generate(&mut cmd, "ngopilot", &mut buffer);

        let script = String::from_utf8(buffer).expect("utf8");
        assert!(script.contains("module completions"));
        assert!(script.contains("export extern ngopilot"));
    ...
    fn term_init_help_mentions_nushell() {
        ...
        assert!(help.contains("ngopilot term init nu"));
    ...
    fn skills_command_accepts_list_subcommand() {
        let cli = Cli::try_parse_from(["ngopilot", "skills", "list"]).expect("parse failed");
    ...
    fn tui_command_accepts_trailing_args() {
        let cli = Cli::try_parse_from(["ngopilot", "tui", "--", "--theme", "dark"])
            .expect("parse failed");
