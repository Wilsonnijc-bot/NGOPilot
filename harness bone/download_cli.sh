...
#   NGOPILOT_BIN_DIR  - Directory to which ngopilot will be installed (default: $HOME/.local/bin)
#   NGOPILOT_VERSION  - Optional: specific version to install (e.g., "v1.0.25"). Overrides CANARY. Can be in the format vX.Y.Z, vX.Y.Z-suffix, or X.Y.Z
#   NGOPILOT_PROVIDER - Optional: provider for ngopilot
#   NGOPILOT_MODEL    - Optional: model for ngopilot
#   NGOPILOT_LINUX_VARIANT - Optional: Linux package variant to install (`standard`, `vulkan`, or `musl`)
#   NGOPILOT_WINDOWS_VARIANT - Optional: Windows package variant to install (`standard` or `cuda`)
#   CANARY         - Optional: if set to "true", downloads from canary release instead of stable
#   CONFIGURE      - Optional: if set to "false", disables running ngopilot configure interactively
...
if ! command -v curl >/dev/null 2>&1; then
  echo "Error: 'curl' is required to download ngopilot. Please install curl and try again."
  exit 1
fi

# Check for tar or unzip (depending on OS)
if ! command -v tar >/dev/null 2>&1 && ! command -v unzip >/dev/null 2>&1; then
  echo "Error: Either 'tar' or 'unzip' is required to extract ngopilot. Please install one and try again."
  exit 1
fi
...
if [[ "${WINDIR:-}" ]] || [[ "${windir:-}" ]] || [[ "$OSTYPE" == "msys" ]] || [[ "$OSTYPE" == "cygwin" ]]; then
    ...
    DEFAULT_BIN_DIR="$USERPROFILE/ngopilot"
else
    # Linux, macOS, and WSL all use the same bin directory
    DEFAULT_BIN_DIR="$HOME/.local/bin"
fi

NGOPILOT_BIN_DIR="${NGOPILOT_BIN_DIR:-$DEFAULT_BIN_DIR}"
RELEASE="${CANARY:-false}"
CONFIGURE="${CONFIGURE:-true}"
NGOPILOT_LINUX_VARIANT="${NGOPILOT_LINUX_VARIANT:-}"
NGOPILOT_WINDOWS_VARIANT="${NGOPILOT_WINDOWS_VARIANT:-standard}"
if [ -n "${NGOPILOT_VERSION:-}" ]; then
  # Validate the version format
  if [[ ! "$NGOPILOT_VERSION" =~ ^v?[0-9]+\.[0-9]+\.[0-9]+(-.*)?$ ]]; then
    echo "[error]: invalid version '$NGOPILOT_VERSION'."
    echo "  expected: semver format vX.Y.Z, vX.Y.Z-suffix, or X.Y.Z"
    exit 1
  fi
  NGOPILOT_VERSION=$(echo "$NGOPILOT_VERSION" | sed 's/^v\{0,1\}/v/') # Ensure the version string is prefixed with 'v' if not already present
  RELEASE_TAG="$NGOPILOT_VERSION"
else
  # If NGOPILOT_VERSION is not set, use stable unless canary was requested.
...
case "$OS" in
  ...
  mingw*|msys*|cygwin*)
    ...
    ;;
  *)
    echo "Error: Unsupported OS '$OS'. ngopilot currently supports Linux, macOS, and Windows."
    exit 1
    ;;
esac
...
if [ "$OS" = "linux" ] && [ -z "$NGOPILOT_LINUX_VARIANT" ]; then
  if detect_linux_musl; then
    NGOPILOT_LINUX_VARIANT="musl"
  else
    NGOPILOT_LINUX_VARIANT="standard"
  fi
elif [ -z "$NGOPILOT_LINUX_VARIANT" ]; then
  NGOPILOT_LINUX_VARIANT="standard"
fi
...
if [ "$OS" = "darwin" ]; then
  FILE="ngopilot-$ARCH-apple-darwin.tar.bz2"
  EXTRACT_CMD="tar"
elif [ "$OS" = "windows" ]; then
  case "$NGOPILOT_WINDOWS_VARIANT" in
    standard|cuda) ;;
    *)
      echo "Error: Unsupported NGOPILOT_WINDOWS_VARIANT '$NGOPILOT_WINDOWS_VARIANT'. Expected 'standard' or 'cuda'."
      exit 1
      ;;
  esac
  ...
  FILE="ngopilot-$ARCH-pc-windows-msvc.zip"
  if [ "$NGOPILOT_WINDOWS_VARIANT" = "cuda" ]; then
    FILE="ngopilot-$ARCH-pc-windows-msvc-cuda.zip"
  fi
  EXTRACT_CMD="unzip"
  OUT_FILE="ngopilot.exe"
else
  case "$NGOPILOT_LINUX_VARIANT" in
    standard|vulkan|musl) ;;
    *)
      echo "Error: Unsupported NGOPILOT_LINUX_VARIANT '$NGOPILOT_LINUX_VARIANT'. Expected 'standard', 'vulkan', or 'musl'."
      exit 1
      ;;
  esac
  FILE="ngopilot-$ARCH-unknown-linux-gnu.tar.bz2"
  if [ "$NGOPILOT_LINUX_VARIANT" = "vulkan" ]; then
    FILE="ngopilot-$ARCH-unknown-linux-gnu-vulkan.tar.bz2"
  elif [ "$NGOPILOT_LINUX_VARIANT" = "musl" ]; then
    FILE="ngopilot-$ARCH-unknown-linux-musl.tar.bz2"
  fi
  EXTRACT_CMD="tar"
fi
...
# --- 4) Download & extract 'ngopilot' binary ---
...
if ! curl -sLf "$DOWNLOAD_URL" --output "$FILE"; then
  ...
  if ! [ -n "${NGOPILOT_VERSION:-}" ] && [ "${CANARY:-false}" != "true" ]; then
...
TMP_DIR="/tmp/ngopilot_install_$RANDOM"
...
# Windows releases may contain files in a 'ngopilot-package' subdirectory
EXTRACT_DIR="$TMP_DIR"
if [ "$OS" = "windows" ] && [ -d "$TMP_DIR/ngopilot-package" ]; then
  echo "Found ngopilot-package subdirectory, using that as extraction directory"
  EXTRACT_DIR="$TMP_DIR/ngopilot-package"
fi

# Make binary executable
if [ "$OS" = "windows" ]; then
  chmod +x "$EXTRACT_DIR/ngopilot.exe"
else
  chmod +x "$EXTRACT_DIR/ngopilot"
fi

# --- 5) Install to $NGOPILOT_BIN_DIR ---
if [ ! -d "$NGOPILOT_BIN_DIR" ]; then
  echo "Creating directory: $NGOPILOT_BIN_DIR"
  mkdir -p "$NGOPILOT_BIN_DIR"
fi

echo "Moving ngopilot to $NGOPILOT_BIN_DIR/$OUT_FILE"
if [ "$OS" = "windows" ]; then
  mv "$EXTRACT_DIR/ngopilot.exe" "$NGOPILOT_BIN_DIR/$OUT_FILE"
else
  ...
  if [ -f "$NGOPILOT_BIN_DIR/$OUT_FILE" ]; then
    mv "$NGOPILOT_BIN_DIR/$OUT_FILE" "$NGOPILOT_BIN_DIR/$OUT_FILE.old"
    if ! mv "$EXTRACT_DIR/ngopilot" "$NGOPILOT_BIN_DIR/$OUT_FILE"; then
      echo "Error: failed to install new binary, restoring previous version"
      mv "$NGOPILOT_BIN_DIR/$OUT_FILE.old" "$NGOPILOT_BIN_DIR/$OUT_FILE"
      exit 1
    fi
    rm -f "$NGOPILOT_BIN_DIR/$OUT_FILE.old"
  else
    mv "$EXTRACT_DIR/ngopilot" "$NGOPILOT_BIN_DIR/$OUT_FILE"
  fi
fi
...
if [ "$CONFIGURE" = true ]; then
  # --- 6) Configure NGOPilot (Optional) ---
  if [ -n "${NGOPILOT_PROVIDER:-}" ]; then
    export GOOSE_PROVIDER="$NGOPILOT_PROVIDER"
  fi
  if [ -n "${NGOPILOT_MODEL:-}" ]; then
    export GOOSE_MODEL="$NGOPILOT_MODEL"
  fi
  echo ""
  echo "Configuring NGOPilot"
  echo ""
  if [ -t 0 ]; then
    "$NGOPILOT_BIN_DIR/$OUT_FILE" configure
  elif [ -r /dev/tty ]; then
    "$NGOPILOT_BIN_DIR/$OUT_FILE" configure < /dev/tty
  else
    echo "Non-interactive shell detected (e.g. 'curl ... | bash')."
    echo "Skipping 'ngopilot configure' — please run it manually after installation:"
    echo "    $NGOPILOT_BIN_DIR/$OUT_FILE configure"
  fi
else
  echo "Skipping 'ngopilot configure', you may need to run this manually later"
fi



# --- 7) Check PATH and give instructions if needed ---
if [[ ":$PATH:" != *":$NGOPILOT_BIN_DIR:"* ]]; then
  echo ""
  echo "Warning: NGOPilot is installed, but $NGOPILOT_BIN_DIR is not in your PATH."

  if [ "$OS" = "windows" ]; then
    echo "To add ngopilot to your PATH in PowerShell:"
    echo ""
    ...
    echo "    ngopilot configure"
    echo "or rerun this install script after updating your PATH."
  else
    SHELL_NAME=$(basename "$SHELL")

    echo ""
    echo "The \$NGOPILOT_BIN_DIR is not in your PATH."
    ...
    if [ "$CONFIGURE" = true ]; then
      ...
      1)
        ...
        echo "Adding \$NGOPILOT_BIN_DIR to $RC_FILE..."
        echo "export PATH=\"$NGOPILOT_BIN_DIR:\$PATH\"" >> "$RC_FILE"
      ...
      2)
        ...
        echo "    export PATH=\"$NGOPILOT_BIN_DIR:\$PATH\""
        echo "Then reload your shell (e.g. 'source ~/.${SHELL_NAME}rc') to apply changes."
        ;;
      *)
        echo "Invalid choice. Please add \$NGOPILOT_BIN_DIR to your PATH manually."
        ;;
      esac
    else
      echo ""
      echo "Configure disabled. Please add \$NGOPILOT_BIN_DIR to your PATH manually."
    fi
