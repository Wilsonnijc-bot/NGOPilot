...
const MENU_TRANSLATIONS_ZH_CN: Record<string, string> = {
  ...
  'Focus NGOPilot Window': '聚焦 NGOPilot 窗口',
  'Quick Launcher': '快速启动器',
  'Always on Top': '窗口置顶',
  'Toggle Navigation': '切换导航',
  'About NGOPilot': '关于 NGOPilot',
...
const DEEP_LINK_PREFIX = 'ngopilot://';

function findDeepLink(args: string[]): string | undefined {
  return args.find((arg) => arg.startsWith(DEEP_LINK_PREFIX));
}
...
if (MAIN_WINDOW_VITE_DEV_SERVER_URL) {
  ...
  console.log('[Main] Development mode: registering NGOPilot deep-link protocols');
  app.setAsDefaultProtocolClient('ngopilot');

  if (process.platform === 'darwin') {
    try {
      // Reset the default handler to ensure dev version takes precedence
      spawn('open', ['-a', process.execPath, '--args', '--reset-protocol-handler', 'ngopilot'], {
        detached: true,
        stdio: 'ignore',
      });
    } catch {
...
if (process.platform !== 'darwin') {
  ...
  if (!gotTheLock) {
    ...
    app.on('second-instance', (_event, commandLine) => {
      const protocolUrl = findDeepLink(commandLine);
      if (protocolUrl) {
  ...
  const protocolUrl = findDeepLink(process.argv);
  if (protocolUrl) {
    app.whenReady().then(async () => {
      let parsedUrl: URL;
      try {
...
async function createResumeChatWindow(parsedUrl: URL, dir?: string): Promise<boolean> {
  ...
  if (!resumeSessionId) {
    log.warn('[Main] Ignoring NGOPilot resume URL without a session id');
    return false;
  }
...
const getExternalBackendUrlFromEnv = (): string | null => {
  if (!process.env.NGOPILOT_EXTERNAL_BACKEND) {
    return null;
  }

  const configuredUrl = process.env.NGOPILOT_EXTERNAL_BACKEND_URL?.trim();
  if (configuredUrl) {
...
const getExternalBackendFromEnv = (): ExternalBackend | null => {
  ...
  const secret = process.env.NGOPILOT_SERVER__SECRET_KEY;
  if (!secret) {
    throw new Error(
      'NGOPILOT_SERVER__SECRET_KEY must be set when using NGOPILOT_EXTERNAL_BACKEND. ' +
  ...
  if (externalBackend) {
    ...
    try {
      ...
      if (!externalBackendReady) {
        ...
        const response = dialog.showMessageBoxSync({
          ...
          detail:
            'The external backend must be running and the configured secret must match NGOPILOT_SERVER__SECRET_KEY on the server.',
    ...
    try {
      ...
      if (!gooseServeResult.certFingerprint) {
        ...
        throw new Error(
          'NGOPilot local service started with TLS but did not return a certificate fingerprint'
        );
      }
      ...
      log.error('NGOPilot local service failed to start', error);
      dialog.showMessageBoxSync({
        type: 'error',
        title: 'NGOPilot Failed to Start',
        message: 'The backend server failed to start.',
        detail: [
          'Backend: NGOPilot local service',
...
async function appMain() {
  ...
  const appMenu = menu?.items.find((item) => item.label === 'NGOPilot');
  if (appMenu?.submenu) {
  ...
  if (fileMenu?.submenu) {
    ...
    if (shortcuts.focusWindow) {
      fileMenu.submenu.append(
        new MenuItem({
          label: menuT('Focus NGOPilot Window'),
  ...
  if (menu && process.platform !== 'darwin') {
    ...
    if (helpMenu.submenu) {
      ...
      // Create the About NGOPilot menu item with a submenu
      const aboutGooseMenuItem = new MenuItem({
        label: menuT('About NGOPilot'),
        submenu: Menu.buildFromTemplate([]), // Start with an empty submenu for About
      });

      // Add the Version menu item (display only) to the About NGOPilot submenu
...
app.whenReady().then(async () => {
  try {
    ...
    dialog.showErrorBox('NGOPilot Error', `Failed to create main window: ${error}`);
    app.quit();
  }
});
