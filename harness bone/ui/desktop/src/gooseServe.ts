...
export const defaultLogger: Logger = {
  info: (...args) => console.log('[ngopilot-serve]', ...args),
  error: (...args) => console.error('[ngopilot-serve]', ...args),
};
...
export const findGooseBinaryPath = (options: FindGooseBinaryOptions = {}): string => {
  ...
  const binaryName = process.platform === 'win32' ? 'ngopilot.exe' : 'ngopilot';
  ...
  throw new Error(
    `NGOPilot binary not found in any of the possible paths: ${possiblePaths.join(', ')}`
  );
};
...
const buildGooseServeEnv = (
  ...
  env.NGOPILOT_SERVER__SECRET_KEY = serverSecret;
...
export const startGooseServe = async ({
  ...
  if (!secretKey) {
    const message = 'NGOPILOT_SERVER__SECRET_KEY is required for the NGOPilot backend';
  ...
  logger.info(`Starting NGOPilot backend from: ${goosePath} on port ${port} in dir ${workingDir}`);
  if (startupTrace) {
  ...
  const onStderrData = (data: Buffer) => {
    ...
    for (const line of lines) {
      if (line.trim() && isFatalError(line)) {
        logger.error(`NGOPilot backend stderr for port ${port} and dir ${workingDir}: ${line}`);
      }
    }
  };
  ...
  gooseProcess.on('exit', (code, signal) => {
    ...
    exitSignal = signal;
    logger.info(
      `NGOPilot backend process exited with code ${code} and signal ${signal} for port ${port} and dir ${workingDir}`
    );
    if (startupTrace) {
  ...
  gooseProcess.on('error', (error) => {
    ...
    logger.error(`Failed to start the NGOPilot backend on port ${port} and dir ${workingDir}`, error);
  ...
  const cleanup = async (): Promise<void> => {
    return new Promise<void>((resolve) => {
      ...
      logger.info('Terminating the NGOPilot backend');
      try {
        if (process.platform === 'win32') {
          if (gooseProcess.pid) {
        ...
        logger.error('Error while terminating the NGOPilot backend process:', error);
      }
  ...
  if (!ready) {
    ...
    throw new Error(
      withStartupDiagnosticsPath(
        `The NGOPilot backend did not become ready on ${statusUrl}.${exitDetails}${stderrDetails}`,
  ...
  if (tls) {
    ...
    if (!fingerprint) {
      ...
      throw new Error(
        withStartupDiagnosticsPath(
          `The NGOPilot backend did not emit a TLS certificate fingerprint on ${statusUrl}.${exitDetails}${stderrDetails}`,
          startupDiagnosticsPath
        )
      );
    }
