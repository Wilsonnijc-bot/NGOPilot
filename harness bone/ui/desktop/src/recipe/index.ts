...
export async function generateDeepLink(recipe: Recipe): Promise<string> {
  ...
  return `ngopilot://recipe?config=${encoded}`;
}
...
export async function parseDeeplink(deeplink: string): Promise<Recipe | null> {
  try {
    ...
    if (!cleanLink.startsWith('ngopilot://recipe?config=')) {
      throw new Error('Invalid deeplink format. Expected: ngopilot://recipe?config=...');
    }

    const recipeEncoded = cleanLink.replace('ngopilot://recipe?config=', '');
