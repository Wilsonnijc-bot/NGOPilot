...
export const GOOSE_SERVE_EXITED_USER_MESSAGE =
  "This window's NGOPilot backend stopped. Close this window and open a new chat to start a new backend. If this keeps happening, restart NGOPilot Desktop.";
...
export class GooseServeLeaseRegistry {
  ...
  create(result: GooseServeResult, secretKey: string): GooseServeLease {
    ...
    const markExited = ({
      ...
      if (logUnexpected && firstExit && !lease.cleanedUp) {
        this.logger.error('NGOPilot ACP server exited unexpectedly', {
          code: lease.exitCode,
  ...
  async cleanupLease(lease: GooseServeLease) {
    ...
    try {
      ...
      this.logger.error('Failed to clean up the NGOPilot backend:', error);
    }
  }
