    ...
    if (isUsingGitHubFallback) {
      ...
      try {
        ...
        const dialogResult = (await dialog.showMessageBox({
          ...
          detail: `The update has been downloaded and extracted. To complete the installation:\n\n1. Click "Open Folder" to view the new NGOPilot.app\n2. Quit NGOPilot (this app will close)\n3. Drag the new NGOPilot.app to your Applications folder\n4. Replace the existing app when prompted\n\nThe update will be available the next time you launch NGOPilot.`,
...
export function setupAutoUpdater(tray?: Tray) {
  ...
  // Respect NGOPILOT_DISABLE_AUTO_DOWNLOAD env var (takes precedence over user setting)
  const envDisabled =
    process.env.NGOPILOT_DISABLE_AUTO_DOWNLOAD === '1' ||
    process.env.NGOPILOT_DISABLE_AUTO_DOWNLOAD === 'true';
  if (envDisabled) {
    autoDownloadDisabled = true;
    log.info('Auto-download disabled via NGOPILOT_DISABLE_AUTO_DOWNLOAD environment variable');
  }
  ...
  autoUpdater.on('update-downloaded', (info: UpdateInfo) => {
    ...
    const notification = new Notification({
      title: 'Update Ready',
      body: `Version ${info.version} will be installed when you quit NGOPilot. Click to install now.`,
    });
    notification.show();
...
function updateTrayIcon(hasUpdate: boolean) {
  ...
  if (hasUpdate) {
    ...
    trayRef.setToolTip('NGOPilot - Update Available');
  } else {
    // Use normal icon
    if (isDev) {
