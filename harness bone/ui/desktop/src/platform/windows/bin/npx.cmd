...
if not defined GOOSE_NODE_DIR (
    SET "GOOSE_NODE_DIR=%LOCALAPPDATA%\NGOPilot\node"
)
...
echo [NGOPilot] Node.js not found. Downloading portable Node.js v%NODE_VERSION%... 1>&2

SET "NODE_ZIP=%TEMP%\ngopilot-node-%NODE_VERSION%.zip"
SET "NODE_EXTRACT=%TEMP%\ngopilot-node-extract"
...
if errorlevel 1 (
    echo [NGOPilot] ERROR: Failed to download Node.js. Please install manually from https://nodejs.org/ 1>&2
...
if exist "%GOOSE_NODE_DIR%\npx.cmd" (
    echo.>"%GOOSE_NODE_DIR%\node-v%NODE_VERSION%.installed"
    SET "PATH=%GOOSE_NODE_DIR%;!PATH!"
    echo [NGOPilot] Node.js v%NODE_VERSION% ready. 1>&2
    "%GOOSE_NODE_DIR%\npx.cmd" %*
    exit /b !errorlevel!
)

echo [NGOPilot] ERROR: Installation failed. Please install Node.js manually from https://nodejs.org/ 1>&2
exit /b 1
